...
def hash_format(input_string):
    """Converts input to a valid SHA-256 512-bit block (16 words)."""
    if isinstance(input_string, list):
        input_string = hash_unformat(input_string)  #  Ensure it's a string

    input_string = input_string.encode().hex()  #  Convert to hex first (only if it's a string)
    input_block = []

    # Split into 32-bit words (8 hex characters each)
    for i in range(0, len(input_string), 8):
        input_block.append(int("0x" + input_string[i:i + 8], 16))

    #  Ensure at least 16 words (512-bit block)
    while len(input_block) < 16:
        input_block.append(0x00000000)  # Pad with zeros if too short

    return input_block[:16]  # Ensure exactly 16 words

def hash_unformat(input_array):
    """Converts a list of hash output integers back to a string for reuse."""
    if isinstance(input_array, str):
        return input_array  #  If already a string, return it directly

    return ''.join([f"{x:08x}" for x in input_array])  #  Convert list of integers to hex string

def F_global(input_string):
    """Computes SHA-256 hash of the input string."""
    my_sha256 = SHA256()
    my_sha256.init()
    formatted_input = hash_format(input_string)  #  Now ensures 16 words
    my_sha256.next(formatted_input)
    return my_sha256.get_digest()

def PRF_global(seed, index):
    """Pseudo-random function using SHA-256."""
    my_sha256 = SHA256()
    my_sha256.init()
...
def chain_global(X, i, s):
    """Applies function F iteratively for 's' times starting from X."""
    for _ in range(i, i + s):
        X = F_global(hash_unformat(X))  # Ensure X is a valid string
    return X

def wots_pkGen_global(SK_seed, w, length):
    """Generates a WOTS public key."""
    return ''.join([hash_unformat(chain_global(PRF_global(SK_seed, str(i)), 0, w - 1)) for i in range(length)])

# --- Main Function ---
def main():
    input_string = input("Enter the string to hash: ")

    # Compute SHA-256 digest using the global F function
    digest = F_global(input_string)
    digest_hex = [f"0x{value:08x}" for value in digest]

    # Generate chain values using the global chain function
    chain_values = chain_global(input_string, 0, 5)
    chain_hex = [f"0x{value:08x}" for value in chain_values]

    # Generate WOTS public key using the global function
    wots_key = wots_pkGen_global(input_string, 16, 32)

    print("\nSHA-256 Digest:")
    print(digest_hex)
    print("\nChain Values:", chain_hex)
    print("\nWOTS Public Key:", wots_key)

    # Write results to a CSV file
    csv_filename = "hash_results.csv"
    with open(csv_filename, mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["SHA-256 Digest", "Chain Values", "WOTS Public Key"])
        writer.writerow([" ".join(digest_hex), " ".join(chain_hex), wots_key])
